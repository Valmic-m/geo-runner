...
export function SnapshotSummaryCard({ snapshot, onEdit, className }: SnapshotSummaryCardProps) {
  ...
  return (
    <div className={cn('border border-border rounded-xl bg-surface p-4', className)}>
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-semibold text-sm text-text">Client Snapshot</h3>
        <button
          onClick={onEdit}
          className="flex items-center gap-1.5 text-xs font-medium text-primary hover:text-primary-dark transition-colors px-2.5 py-1.5 rounded-lg hover:bg-primary/5"
        >
          <Pencil size={13} />
          Edit
        </button>
      ...
      <div className="flex items-center gap-2 mb-3">
        <span className="text-base font-bold text-text">{snapshot.businessName || 'Unnamed Business'}</span>
      </div>
